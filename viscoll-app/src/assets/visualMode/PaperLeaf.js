...
PaperLeaf.prototype = {
  ...
  draw: function () {
    ...
    const leafNotesToShow = this.leaf.notes
      .filter(noteID => {
        return this.Notes[noteID].show;
      })
      .reverse();
    const rectoNotesToShow = this.recto.notes
      .filter(noteID => {
        return this.Notes[noteID].show;
      })
      .reverse();
    const versoNotesToShow = this.verso.notes.filter(noteID => {
      return this.Notes[noteID].show;
    });
    ...
    for (let noteIndex = 0; noteIndex < rectoNotesToShow.length; noteIndex++) {
      ...
      const noteTitle = this.folio_number
        ? '▼ ' + this.folio_number + ' : ' + note.title.substr(0, numChars)
        : '▼ R : ' + note.title.substr(0, numChars);
      let textNote = new paper.PointText({
        content: noteTitle,
        point: [
          textX,
          textY - noteIndex * (this.spacing * 0.7) - this.spacing * 0.3,
        ],
    ...
    for (let noteIndex = 0; noteIndex < versoNotesToShow.length; noteIndex++) {
      ...
      const noteTitle = this.folio_number
        ? '▲ ' + this.folio_number + ' : ' + note.title.substr(0, numChars)
        : '▲ V : ' + note.title.substr(0, numChars);
      let textNote = new paper.PointText({
        content: noteTitle,
        point: [
          textX,
          this.y + noteIndex * (this.spacing * 0.7) + this.spacing * 0.8,
        ],
        fillColor: this.strokeColor,
        fontSize: fontSize,
      });
  ...
  showAttributes: function () {
    ...
    if (this.visibleAttributes.leaf) {
      if (this.visibleAttributes.leaf.folio_number) {
        if (this.folio_number) leafValues.push(this.folio_number);
      }
    }
